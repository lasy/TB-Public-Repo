...
ntt = 0;
  ...
  //printf("before dysta cjratio: %3.2f\n", cjratio[0]);
  dysta2(*nsam, *jpp, nsel, x, *n, dys, *diss_kind,
         jtmd, valmd, has_NA, &dyst_toomany_NA, w, cjratio);
  if(dyst_toomany_NA) {
    if(*trace_lev)
...
dysta2(*nsam, *jpp, nbest, x, *n, dys, *diss_kind, jtmd, valmd,
       has_NA, &dyst_toomany_NA, w, cjratio);
if(dyst_toomany_NA) {
  error(_(
...
#undef ntmp6
        ...
        if (x[lj] == valmd[j] || x[kj] == valmd[j]) {
          continue /* next j */;
              ...
              if (diss_kind == EUCLIDEAN)
              ...
              else if (diss_kind == CUSTOM_JACCARD){
                //printf("j / jpp / n : %i / %i / %i% \n",j,jpp, n);
                //printf("lj: %i  -- kj: %i  \n",lj, kj);
                
                // tracking behavior
                if((x[lj] >= 0. && x[kj] >= 0.) || (x[lj] < 0. && x[kj] < 0.)) { clk2+=w[j] ;} ; // both have same tracking - increment numerator
                N_ones2+=w[j];
                ...
                double sx_k = x[kj];
                if(sx_k < 0){sx_k = 0;}
                double sx_l_prev = sx_l;
                double sx_l_next = sx_l;
                double sx_k_prev = sx_k;
                double sx_k_next = sx_k;
                if ((j > 0)){
                  sx_l_prev = x[lj-n];
                  if(sx_l_prev < 0){sx_l_prev = 0;}
                  sx_k_prev = x[kj-n];
                  if(sx_k_prev < 0){sx_k_prev = 0;}
                }
                if(j < (jpp-1)){
                  sx_l_next = x[lj+n];
                  if(sx_l_next < 0){sx_l_next = 0;}
                  sx_k_next = x[kj+n];
                  if(sx_k_next < 0){sx_k_next = 0;}
                }
                sx_l = 0.25*sx_l_prev + 0.5 * sx_l + 0.25*sx_l_next;
                sx_k = 0.25*sx_k_prev + 0.5 * sx_k + 0.25*sx_k_next;
                
                if( sx_l > 0. && sx_k > 0.) { // both have TB - increment numerator and denominator
                  if (sx_l <  sx_k){ clk += w[j]*sx_l; N_ones +=  w[j]*sx_k; } else {clk += w[j]*sx_k;  N_ones +=  w[j]*sx_l; }
                } else if( sx_l > 0.)  // A (l) only has TB - increment N_ones
                  N_ones+= w[j]*sx_l ;
                else if( sx_k > 0.)  // B (k) only has TB - increment N_ones
                  N_ones+= w[j]*sx_k ;
              }
        ...
        *toomany_NA = TRUE;
          ...
          //printf("N_ones2: %3.2f\n",N_ones2);
          //printf("cjratio: %3.2f\n", cjratio[0]);
          //printf("d: %3.2f\n", 1 - cjratio[0] * clk / N_ones - (1-cjratio[0])*clk2 / N_ones2);
          if((diss_kind ==  CUSTOM_JACCARD) && (N_ones == 0)){clk = clk2/2 ; N_ones = N_ones2;}
          double d1 = clk * (jpp / (double) npres);
          dys[nlk] =
            (diss_kind == EUCLIDEAN) ? sqrt(d1)
            :(diss_kind ==  JACCARD) ? 1 - clk / N_ones
            :(diss_kind ==  CUSTOM_JACCARD) ? 1 - cjratio[0] * clk / N_ones - (1-cjratio[0])*clk2 / N_ones2
              ...
              double *ttnew, double *rdnew, int correct_d, double *w, double *cjratio)
              {
                ...
                *zb = 0.;
                ...
                for(i = 1; i <= n; i++) {
                  ...
                  if (!has_NA) {
                    for (jk = 1; jk <= kk; ++jk) {
                      ...
                      if (nr_k != i) {
                        double N_ones = 0;
                        double N_ones2 = 0; // init only for CUSTOM_JACCARD
                        double tra = 0.; // init only for JACCARD
                        double tra2 = 0.; // init only for CUSTOM_JACCARD
                        for (jp = 0; jp < jpp; ++jp) {
                          na = (nr_k - 1) + jp * n;
                          nb = (i    - 1) + jp * n;
                          if (diss_kind == JACCARD) {
                            ...
                            // tracking behavior
                            if(( x[na] >= 0. && x[nb] >= 0.) || ( x[na] < 0. && x[nb] < 0.)) { tra2+=w[jp] ;}// both have same tracking - increment numerator
                            N_ones2+=w[jp] ;
                            ...
                            double sx_b = x[nb];
                            if(sx_b < 0){sx_b = 0;}
                            double sx_a_prev = sx_a;
                            double sx_a_next = sx_a;
                            double sx_b_prev = sx_b;
                            double sx_b_next = sx_b;
                            
                            if (j > 0){
                              sx_a_prev = x[na-n];
                              if(sx_a_prev < 0){sx_a_prev = 0;}
                              sx_b_prev = x[nb-n];
                              if(sx_b_prev < 0){sx_b_prev = 0;}
                            }
                            if(j < (jpp-1)){
                              sx_a_next = x[na+n];
                              if(sx_a_next < 0){sx_a_next = 0;}
                              sx_b_next = x[nb+n];
                              if(sx_b_next < 0){sx_b_next = 0;}
                            }
                            sx_a = 0.25*sx_a_prev + 0.5 * sx_a + 0.25*sx_a_next;
                            sx_b = 0.25*sx_b_prev + 0.5 * sx_b + 0.25*sx_b_next;
                        
                            if( sx_a > 0. && sx_b > 0.) { // both have TB - increment numerator and denominator
                              if (sx_a <  sx_b){ tra += w[jp]*sx_a; N_ones +=  w[j]*sx_b; } else {tra += w[jp]*sx_b;  N_ones +=  w[jp]*sx_a; }
                            } else if( sx_a > 0.)  // A (l) only has TB - increment N_ones
                              N_ones+= w[jp]*sx_a ;
                            else if( sx_b > 0.)  // B (k) only has TB - increment N_ones
                              N_ones+= w[jp]*sx_b ;
                        ...
                        if (diss_kind == JACCARD)
                          dsum = 1 - tra / N_ones;
                        if (diss_kind == CUSTOM_JACCARD){
                          if(N_ones == 0){tra = tra2/2; N_ones = N_ones2;}
                          dsum = 1 - tra / N_ones * cjratio[0] - tra2 / N_ones2 * (1-cjratio[0]);
                          //printf("tra: %3.2f\n",tra);
                          //printf("N_ones: %3.2f\n",N_ones);
                          //printf("tra2: %3.2f\n",tra2);
                          //printf("N_ones2: %3.2f\n",N_ones2);
                    ...
                    for (jk = 1; jk <= kk; ++jk) {
                      ...
                      if (nr_k != i) {
                        int nobs = 0, N_ones = 0;
                        int N_ones2 = 0; // init only for CUSTOM_JACCARD
                        double tra = 0.; // init only for JACCARD
                        double tra2 = 0.; // init only for CUSTOM_JACCARD
                        for (jp = 0; jp < jpp; ++jp) {
                          na = (nr_k - 1) + jp * n;
                ...
                continue /* next jk */;
                        ...
                        if (diss_kind == CUSTOM_JACCARD)
                          if(N_ones == 0){tra = tra2/2 ; N_ones = N_ones2;}
                          dsum = 1 - tra / (double)N_ones * cjratio[0] - tra2 / (double)N_ones2 * (1-cjratio[0]);
