...
if(length(stretch_ids)>0){
  ...
  stretches$TB_change_cat = cut(stretches$TB_ratio, 
                                breaks = c(-Inf, 2^c( -10, -1.5,-0.5,0.5,1.5,10),Inf), 
  ...
  table_ratio = table(round(log2(stretches$TB_ratio[stretches$valid_transition])))
  table_ratio
  
  round(100*table_ratio/sum(table_ratio),1)
  
  table_ratio_x_BC = table(stretches$BC, round(log2(stretches$TB_ratio)))
  table_ratio_x_BC
  round(100*table_ratio_x_BC/apply(table_ratio_x_BC,1,sum),1)
  
  
  table_TB_change_cat_x_BC = table(stretches$BC, stretches$TB_change_cat)
