...
```{r users_descr_cluster_info save cycles_m}
save(cycles_m, file = paste0(IO$output_data, "cycles_m.Rdata"))
file.copy(from = paste0(IO$output_data, "cycles_m.Rdata"), to = paste0(IO$tmp_data, "cycles_m_with_clusters_info.Rdata"))
```
...
users$n_cycles_with_TB_m = apply(users[,j],1,sum)
...
users$n_cycles_m = agg$n_cycles_m[match(users$user_id, agg$user_id)]

users$perc_cycles_with_TB_m = users$n_cycles_with_TB_m / users$n_cycles_m
