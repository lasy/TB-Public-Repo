...
- for users that have cycles with different labels, we keep the longuonly take users for which all cycles have high enough confidence score for the new labels and for which the average confidence over that sequence of consecutive cycle is high enough as well (twice higher the confidence score for individual cycles).
...
j_same = which(cycles$user_id %in% user_ids_same_label)
cycles$BC[j_same] = cycles$BC_hmm_init[j_same]

if(length(j_same)>0){cycles_remain = cycles[-j_same,]}else{cycles_remain = cycles}
...
cycles$BC[cycles$user_id %in% user_ids] = cycles$BC_hmm_init[cycles$user_id %in% user_ids]
