...
cycles = read_feather(paste0(IO$output_data, "cycles.feather"))
...
agg = agg %>% mutate(tracking_group = recode(tracking_group, `pre-menstrual tracking` = "peri-menstrual tracking"))
...
agg = df %>% dplyr::select(BC, cat, names, n_users, n_stretches, tot_stretches, tot_users) %>% 
  group_by(cat, names) %>% 
  summarize_each(funs = sum,  n_users, n_stretches, tot_stretches, tot_users) %>% 
  mutate(perc_stretches = n_stretches/tot_stretches * 100,
         perc_users = n_users/ tot_users *100,
         BC = "all")

df$BC = as.character(df$BC)

df_stat = rbind(
  df %>% dplyr::select(BC, cat, names, n_users, tot_users, perc_users, n_stretches, tot_stretches, perc_stretches) %>% as.data.frame(),
  agg %>% dplyr::select(BC, cat, names, n_users, tot_users, perc_users, n_stretches, tot_stretches, perc_stretches) %>% as.data.frame()
)


df_stat$BC = factor(df_stat$BC, levels = c(as.character(par$BC_dict$name), "all"))

g_stat= ggplot(df_stat, aes(x = as.factor(cat), y = perc_stretches/100, fill = BC))+
  coord_flip()+
  geom_bar(stat = "identity", position = "dodge")+
...
g_stat= ggplot(df, aes(x = factor(cat, levels = rev(sort(unique(cat)))), y = perc_users/100, fill = BC))+
  ...
  geom_text(aes(label = round(perc_users,2), col = BC, vjust = ifelse(BC == "pill",-1.5,2.5)), hjust = 0, nudge_y = 0.005 * 1)+
  scale_color_manual(values = cols$BC)+
  scale_fill_manual(values = cols$BC)+
...
g_stat= ggplot(df_stat, aes(x = factor(cat, levels = rev(sort(unique(cat)))), y = perc_users/100, fill = BC))+
  coord_flip()+
  geom_bar(stat = "identity", position = "dodge", width = 0.8)+
  geom_text(aes(label = round(perc_users,2), col = BC, vjust = (2.25-match(BC, levels(df_stat$BC)))*2.5 ), hjust = 0, nudge_y = 0.005 * 1)+
  scale_color_manual(values = cols$BC3)+
  scale_fill_manual(values = cols$BC3)+
  guides(fill = FALSE, col = FALSE)+
  scale_y_continuous(labels = scales::percent)+
  scale_x_discrete(labels = NULL, breaks = NULL)+
...
# Clinical significance of age on the symptoms prevalence
...
cvdt_stretches$age_cat = users$age_cat[match(cvdt_stretches$user_id, users$user_id)]


ggplot(cvdt_stretches, aes(x = age_cat, y = duration, fill = age_cat)) +
  geom_boxplot()+
  facet_grid(. ~ BC, scale = "free")

ggplot(cvdt_stretches, aes(x = age_cat, y = timing, fill = age_cat)) +
  geom_boxplot()+
  facet_grid(. ~ BC, scale = "free")

ggplot(cvdt_stretches, aes(x = age_cat, y = variability, fill = age_cat)) +
  geom_boxplot()+
  facet_grid(. ~ BC, scale = "free")


ggplot(cvdt_stretches, aes(x = age_cat, y = cyclicity, fill = age_cat)) +
  geom_boxplot()+
  facet_grid(. ~ BC, scale = "free")


aggregate(duration ~ age_cat + BC,  cvdt_stretches, mean)

aggregate(timing ~ age_cat + BC,  cvdt_stretches, mean)


aggregate(variability ~ age_cat + BC,  cvdt_stretches, mean)

aggregate(cyclicity ~ age_cat + BC,  cvdt_stretches, mean)



cvdt_stretches$severe = (cvdt_stretches$duration >= 20)
severity = ddply(cvdt_stretches,
                 .(age_cat, BC),
                 summarize,
                 n_severe = sum(severe),
                 n = length(severe),
...
ggplot(severity, aes(x = age_cat, y = f_severe, col = age_cat ))+
  geom_point()+
  facet_grid(. ~ BC)
...
ggplot(cycles_m %>%  filter((tracking_group == "regular tracking") & (BC == "none / condoms")),
       aes(x = n_TB, fill = age_cat))+
  geom_histogram(binwidth = 1)+
  facet_grid(age_cat ~ BC, scale = "free")
