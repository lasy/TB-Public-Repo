...
if((nrow(sel_d_inconsistent)>0)&(nrow(sel_d_consistent)>0)){
  ...
  grid.arrange(g_consistent, g_average, g_inconsistent, nrow = 1)
}
...
user_ids = 
  c("a425726839dbff5f10272c564486b5a8ec4a3862", "79f4a938b9e8373c1d0b8ea5cb75177fc371045a", 
    "4e176a95b8e1a597b3450da70decf84eb0c0af79", "6ef947aeb9cfe0468bd1efd26b8a10dca0a1e645", 
    "17115bfd58d8ec029cdc33f6baca4b9f3facf4f5","a992ef487ea4400458f13fd658b84cdec0a89e78",
    "30f5a242cbc40cd7430a1077907d806ed24e3ba1","d63bb13e24e6ee03bac73670e13646df708c37c6",
    "c154cdb272a63d259667755834768dd7b77c1306","2e30d0a5b7c2c48993920c63485bc187c521e838",
    "f38a635f9ca89b5258e37049d2933b9ef8086d58","d915a3e8ea4abefc864ef25686a9739eb9f87072")
...
if((nrow(sel_d_inconsistent)>0)&(nrow(sel_d_consistent)>0)&(nrow(sel_d_average)>0)){
  ...
  g_consistent = ggplot_imputed_TB(sel_d = sel_d_consistent, 
                                   facet_grid = c("BC","user_id"), cycle_id = FALSE, col = "BC")+
  ...
  grid.arrange(g_consistent, g_average, g_inconsistent, nrow = 1)
  g_consistency = arrangeGrob(g_consistent, g_average, g_inconsistent, nrow = 1, widths = c(1,1,1))
  g_consistency
  
  ggsave(filename = paste0(IO$panels, "consistency_examples.pdf"), 
         plot = g_consistency, 
         width = viz$full_width, 
         height = 0.4*viz$full_width,
         scale = viz$scale)
}
...
g = ggplot(df, aes(x = consistency, y = n_users, fill = BC)) + 
  geom_area(position = "identity", alpha = 0.75) + 
  facet_grid(BC ~ init_TB_group, scale = "free_y")+
