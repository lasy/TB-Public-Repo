...
file.copy(from = paste0(IO$output_data, "users.Rdata"), to =  paste0(IO$output_data, "users_with_demographics.Rdata"), overwrite = TRUE)



save(cycles, file = paste0(IO$output_data, "cycles.Rdata"))
file.copy(from = paste0(IO$output_data, "cycles.Rdata"), to =  paste0(IO$output_data, "cycles_with_demographics.Rdata"), overwrite = TRUE)
...
agg_users = aggregate(user_id ~ age_cat + bmi_cat + birth_control, users, length)
colnames(agg_users)[which(colnames(agg_users) == "user_id")] = "n_users"

agg_cycles = aggregate(cycle_id ~ age_cat + bmi_cat + birth_control, cycles, length)
colnames(agg_cycles)[which(colnames(agg_cycles) == "cycle_id")] = "n_cycles"

agg = merge(agg_users, agg_users)
