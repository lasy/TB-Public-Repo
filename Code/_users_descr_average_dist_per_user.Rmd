...
tic()
foreach(n = 1:N,.packages = c("foreach","Rcpp"),.noexport = "TB_distance_Rccp")%dopar%{
...
users$TB_freq = users$n_TB/users$n_cycles_m
...
ggplot(users[user_selection,], aes(x = sd_d, fill = BC)) +
  geom_histogram(position = "identity", col = NA)+
  facet_grid(BC ~ ((users$n_TB/users$n_cycles_m)>=1) )
