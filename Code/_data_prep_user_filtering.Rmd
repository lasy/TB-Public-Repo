...
We want a maximum of 5000 users per batch.

```{r user_filt defining a batch number for each user}
N = 5000
users$batch = rep(c(1:ceiling(nrow(users)/N)),each = N)[1:nrow(users)]
rm(N)
save(users, file = paste0(IO$output_data,"users.Rdata"))
```
...
file.copy(paste0(IO$output_data,"users.Rdata"), paste0(IO$tmp_data,"users_before_filtering_days.Rdata"), overwrite = TRUE)
