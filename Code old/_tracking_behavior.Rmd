...
```{r tracking_behavior setup, include=FALSE}
...
```{r tracking_behavior loading cycles if not already loaded }
if(!exists("cycles")){
load(file = paste0(IO$input_data,"cycles.Rdata"), verbose = TRUE)}
```

## Tracking behavior in each birth control category

```{r tracking behavior n logs per cycle for pill and non-pill users}
...
```{r tracking behavior n logs per cycle for pill and non-pill users including the zeros on one days file}
...
```{r tracking behavior n logs per cycle for pill and non-pill users including the zeros on one days file - new classification}
...
```{r tracking_behavior load one days file as example}
...
```{r tracking_behavior tracking behavior by birth control and cycleday_m_D}
...
```{r tracking_behavior pill taken and period onset}
...
```{r tracking_behavior check if all cycles have bleeding or any log on day 1}
...
```{r tracking_behavior tracking patterns by age and birth control categories}
...
```{r tracking_behavior tracking patterns by age and BC categories}
