	    ...
	    for (int k = 0; k < l; ++k) { /* compute d(nsel[l], nsel[k]) {if possible}*/
	      ...
	      for (j = 0, lj = lsel-1, kj = ksel-1; j < jpp;
		    ...
		    if (diss_kind == EUCLIDEAN)
		      ...
		      // tracking behavior
		      if( x[lj] >= 0. && x[kj] >= 0.) { // both have logs - increment numerator
		        clk2+=w[j] ; N_ones2+=w[j] ;
		      } else if( x[lj] >= 0. || x[kj] >= 0.)// any is 1 - increment N_ones2
		        N_ones2+=w[j] ;
		      // actual symptoms
		      if( x[lj] > 0. && x[kj] > 0.) { // both have TB - increment numerator
		        if (x[lj] <  x[kj]){ clk += w[j]*x[lj]; N_ones +=  w[j]*x[kj]; } else {clk += w[j]*x[kj];  N_ones +=  w[j]*x[lj]; }
		      } else if( x[lj] > 0.)  // any have TB - increment N_ones
		        N_ones+= w[j]*x[lj] ;
		      else if( x[kj] > 0.)  // any have TB - increment N_ones
		        N_ones+= w[j]*x[kj] ;
		    }
	...
	if (!has_NA) {
		...
		if (nr_k != i) {
			...
			if (diss_kind == JACCARD) {
			  ...
			  //printf("jp: %i\n",jp);
			  // tracking behavior
			  if( x[na] >= 0. && x[nb] >= 0.) { // both have logs - increment numerator
			    //printf("%3.2f\n",w[jp]);
			    tra2+=w[jp] ; N_ones2+=w[jp] ;
			  } else if( x[na] >= 0. || x[nb] >= 0.)// any is 1 - increment N_ones2
			    N_ones2+=w[jp] ;
			  // actual symptoms
			  if(x[na] > 0. && x[nb] > 0.) { // both have TB - increment numerator
			    if (x[na] <  x[nb]){ tra += w[jp]*x[na]; N_ones +=  w[jp]*x[nb]; } else {tra += w[jp]*x[nb];  N_ones += w[jp]*x[na]; }
			  } else if( x[na] > 0.)  // any have TB - increment N_ones
			    N_ones+= x[na] ;
			  else if( x[nb] > 0.)  // any have TB - increment N_ones
			    N_ones+= x[nb] ;
